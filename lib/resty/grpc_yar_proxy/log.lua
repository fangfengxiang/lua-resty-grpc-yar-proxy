...
--   true：on_response 仅将 entry 存到 platform.ctx，由 flush_logs() 在 log_by_lua 阶段输出
--         日志 I/O 移出响应热路径，对标 nginx access_log 的 log phase 语义
--
-- @param opts table|nil { level = platform.LOG_INFO, defer = boolean, writer = fn(level, msg) }
...
function _M.access_logger(opts)
    opts = opts or {}
    local level = opts.level or platform.LOG_INFO
    ...
    return {
        on_request = function(_method, params)
            ...
            ctx[CTX_START_TIME] = platform.now()
        ...
        on_response = function(method, retval, err_obj)
            ...
            local entry = {
                ts           = platform.localtime(),
                level        = (status == "ok") and "info" or "warn",
                module       = "grpc_yar_proxy",
                service      = service,
                method       = method or "unknown",
                params_size  = platform.ctx[CTX_PARAMS_SIZE] or 0,
                status       = status,
            ...
            if defer then
                -- 延迟模式：存到 platform.ctx，由 flush_logs() 在 log_by_lua 阶段输出
                platform.ctx[CTX_LOG_ENTRY] = entry
            else
                -- 即时模式：立即输出
                local json = to_json(entry)
                local log_level = (status == "ok") and level or platform.LOG_WARN
                if writer then
                    writer(log_level, json)
                else
                    platform.log(log_level, json)
                end
            end
        end,
    }
...
-- 从 platform.ctx 读取 on_response 组装的 entry 并输出。
-- 若 entry 不存在（非 RPC 请求或未配置 defer 模式），静默返回。
-- @param opts table|nil { writer = fn(level, msg) }，默认 platform.log(platform.LOG_INFO, ...)
---@param opts? table { writer:fun(level:integer,msg:string) }
function _M.flush_logs(opts)
    opts = opts or {}
    local writer = opts.writer

    local entry = platform.ctx[CTX_LOG_ENTRY]
    if not entry then
        return
    end
    local json = to_json(entry)
    local level = (entry.status == "ok") and platform.LOG_INFO or platform.LOG_WARN
    if writer then
        writer(level, json)
    else
        platform.log(level, json)
    end
end
